...
@Component({
  selector: 'app-worldknowledge',
  templateUrl: './worldknowledge.component.html',
  styleUrls: ['./worldknowledge.component.css'],
  animations: [
    trigger('one1', [
      state(
        'one',
...
export class WorldknowledgeComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.http.getWordKnowledge().subscribe((data: Word) => {
      this.word = data;
      if (this.word) {
        let id = this.word.id;
        ...
        if (val == null || val != `${id}`) {
          localStorage.setItem('knowledgeCount', '100');
          localStorage.setItem('id', `${id}`);
        }
        this.count = localStorage.getItem('knowledgeCount');
      }
    });
  }
    ...
    if (answers!.length == 6 && answers != null) {
      ...
      if (this.word?.word != answers) {
        if (Number(this.count) > 0) {
          ...
          localStorage.setItem('knowledgeCount', `${this.count}`);
        }
      }
    }
  }
