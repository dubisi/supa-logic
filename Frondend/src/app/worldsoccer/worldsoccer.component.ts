...
@Component({
  selector: 'app-worldsoccer',
  templateUrl: './worldsoccer.component.html',
  styleUrls: ['./worldsoccer.component.css'],
  animations: [
    trigger('one1', [
      state(
        'one',
...
export class WorldsoccerComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.http.getWord().subscribe((data: Word) => {
      this.word = data;
      if (this.word) {
        let id = this.word.id;
        let val: string | null = localStorage.getItem('id2');
